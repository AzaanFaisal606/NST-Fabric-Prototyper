...
> This is the **baseline (master)** branch. For the two-stage NST refinements
> and the extra colour / split sliders, switch to the `refinements` branch.

---

## Shared components (used by both branches)

These are the building blocks the project always uses.
...
| **Pattern suppression** (bilateral filter on target garment, optional) | If the original target shirt already has a print we don't want, this flattens it before NST starts. (Off by default.) |
...
| **LAB histogram match to the source** | Remaps the stylised image's colour distribution toward the source's actual fabric palette. |
...
The output is at the input target's native resolution.
...
- **Behind the scenes**: when ON, the target garment region (and only
  that region — the mask is used to bound the effect) is run through a
  **bilateral filter** with a large colour sigma, which smooths
  textures while preserving the big folds and silhouette of the shirt.
